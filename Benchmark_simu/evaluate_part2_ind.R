...
eval_result <- readRDS(paste0("compare_simulation/compare_simu_multi_data/evaluation/", data_name, "_eval_IND.rds"))
...
null_cor <- diag(1, nrow=1000, ncol=1000)

ori_dist <- eval_result[["ori"]]
ori_dist$gene_cor <- null_cor[upper.tri(null_cor)]
...
saveRDS(ks_stat, paste0("compare_simulation/compare_simu_multi_data/evaluation/", data_name, "_eval_ks_IND.rds"))

kde_stat <- as.data.frame(kde_stat)
kde_stat$data <- data_name
saveRDS(kde_stat, paste0("compare_simulation/compare_simu_multi_data/evaluation/", data_name, "_eval_kde_IND.rds"))
