...
set.seed(seed)
count_ex <- readRDS(paste0("marginal_fit/",dname,"_NC_Ex_count.rds"))
marginal_fit_fn <- paste0("marginal_fit/",dname,"_NC_Ex_marginal_fit.rds")
...
km_Ex5 <- readRDS(paste0('marginal_fit/',dname,'_NC_Ex_ks_fit_5.rds'))
...
simu_ROSMAP <- NB_copula(mu, gene_name, seq_depth, cell_name, alpha,
                         ind=T, seed=seed)
rm(gene_name)
if(!file.exists(paste0("real/ex/simu_IND/", dname,"/"))){
  dir.create(paste0("real/ex/simu_IND/", dname,"/"),recursive = T)
}
saveRDS(simu_ROSMAP, paste0("real/ex/simu_IND/", dname,"/simu_", prefix, ".rds"))

### estimate correlation --------------------------------------------------------
cor_gene_name <- readRDS("marginal_fit/ROSMAP_NC_Ex_cor_gene.rds")
...
path2 <- paste0("real/ex/simu_IND/noise/",dname,"/",prefix,"/")
...
saveRDS(est_mat_ROSMAP, paste0("real/ex/simu_IND/", dname,"/est_cor_", prefix, ".rds"))
